...
export default function Dashboard() {
  ...
  const statusColor: Record<string, string> = {
    'Order Confirmed': 'bg-gray-100 text-gray-700',
    'Baking in Progress': 'bg-orange-100 text-orange-700',
    'Quality Sealed & Packed': 'bg-blue-100 text-blue-700',
    'Out for Delivery': 'bg-yellow-100 text-yellow-700',
    'Delivered Successfully': 'bg-green-100 text-green-700',
  };
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {recentOrders.map((order) => (
                <tr key={order._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${statusColor[order.orderStatus] || 'bg-gray-100 text-gray-700'}`}>
                      {order.orderStatus}
                    </span>
                  </td>
